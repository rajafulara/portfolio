...
const Work = ({isDarkMode}) => {
  return (
    ...
    id="work" className="w-full px-[12%] py-10 scroll-mt-20">
        ...
        className='grid grid-cols-(--my-grid-cols) my-10 gap-5 dark:text-black'>
            {workData.map((projects, index)=>(
                <motion.div
                whileHover={{scale: 1.05}}
                transition={{duration: 0.3}}
                key={index} className='aspect-square bg-no-repeat bg-cover bg-center rounded-lg relative cursor-pointer group' style={{backgroundImage: `url(${projects.bgImage})`}}>
